...
async function updateAllowFromStoreEntry(params: {
  ...
  const fallbackAllowFrom: AllowFromStore = { version: 1, allowFrom: [] };
...
export async function listChannelPairingRequests(
  ...
  const fallbackPairingStore: PairingStore = { version: 1, requests: [] };
  return await withFileLock(
    filePath,
    fallbackPairingStore,
    async () => {
      ...
      if (expiredRemoved || cappedRemoved) {
        const prunedStore: PairingStore = { version: 1, requests: pruned };
        await writeJsonFile(filePath, prunedStore);
      }
...
export async function upsertChannelPairingRequest(params: {
  ...
  const fallbackUpsertStore: PairingStore = { version: 1, requests: [] };
  return await withFileLock(
    filePath,
    fallbackUpsertStore,
    async () => {
      ...
      if (existingIdx >= 0) {
        ...
        const cappedStore: PairingStore = { version: 1, requests: capped };
        await writeJsonFile(filePath, cappedStore);
      ...
      if (PAIRING_PENDING_MAX > 0 && reqs.length >= PAIRING_PENDING_MAX) {
        if (expiredRemoved || cappedRemoved) {
          const cappedOnlyStore: PairingStore = { version: 1, requests: reqs };
          await writeJsonFile(filePath, cappedOnlyStore);
        }
        return { code: "", created: false };
      }
      ...
      const newStore: PairingStore = { version: 1, requests: [...reqs, next] };
      await writeJsonFile(filePath, newStore);
...
export async function approveChannelPairingCode(params: {
  ...
  const filePath = resolvePairingPath(params.channel, env);
  const fallbackApproveStore: PairingStore = { version: 1, requests: [] };
  return await withFileLock(
    filePath,
    fallbackApproveStore,
    async () => {
      ...
      if (idx < 0) {
        if (removed) {
          const prunedOnlyStore: PairingStore = { version: 1, requests: pruned };
          await writeJsonFile(filePath, prunedOnlyStore);
        }
        return null;
      }
      ...
      pruned.splice(idx, 1);
      const afterApproveStore: PairingStore = { version: 1, requests: pruned };
      await writeJsonFile(filePath, afterApproveStore);
